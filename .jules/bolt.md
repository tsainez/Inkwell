...
## 2024-05-24 - [Caching Computed Properties in SwiftUI Methods]
**Learning:** [Computed properties in Swift, such as array-to-dictionary transformations, re-execute entirely on every access. While converting a repeated computed property lookup into a cached variable in `body` helps, `var body` sub-views (like `var statsAndGoalBanner: some View`) defined in the same view struct implicitly access the instance state and trigger re-evaluation. Thus `O(N)` properties can still be re-evaluated inside these sub-views if they reference the original computed variables rather than the cached `body` ones.]
**Action:** [Cache computationally expensive computed properties into a local variable before using them inside a `body`. To prevent `O(N)` properties from being continuously re-evaluated in extracted view snippets, convert the Snippets into functions that accept the cached properties as arguments (e.g. `func characterTable(filteredCharacters:) -> some View`) rather than implicitly referencing struct-level computed properties.]
